...
class CNN(object):
    ...
    def __init__(self, emb, nf=400, nc=2, de=400, p_drop=0.5, fs=[3,4,5], penalty=0,
        ...
        self.init_emb = theano.shared(name='iWords',
        ...
        if train_emb:
            ...
            L = T.nnet.nnet.categorical_crossentropy(pyx, Y).mean() + penalty*sum([abs(p).sum() for p in self.params]) + ((self.emb - self.init_emb)**2.).sum()
