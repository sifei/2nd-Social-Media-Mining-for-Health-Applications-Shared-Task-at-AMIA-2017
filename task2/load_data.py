...
class ProcessData(object):
    def __init__(self, pretrain_wv=None, lower=True, min_df=5):
	    ...
	    self.wv = gensim.models.KeyedVectors.load_word2vec_format(pretrain_wv,binary=True,unicode_errors='ignore')

        else:
            self.wv = None
        self.embs = [np.zeros((400,)),
            np.random.random((400,))*0.01]
        ...
        for value, key in enumerate(token_cnts):
            ...
            if self.wv is not None:
                if key in self.wv:
                    ...
                    self.embs.append(np.random.random((400,))*0.01)
            else:
                self.embs.append(np.random.random((400,))*0.01)
            index += 1
